...
exports.registerLaunchpadCommands = void 0;
...
const KNOWN_QUOTE_TOKENS = [
    { mint: new web3_js_1.PublicKey("So11111111111111111111111111111111111111112"), symbol: "SOL", decimals: 9 },
    { mint: new web3_js_1.PublicKey("USD1ttGY1N17NEEHLmELoaybftRBUSErhqYiQzvEmuB"), symbol: "USD1", decimals: 6 },
    { mint: new web3_js_1.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"), symbol: "USDC", decimals: 6 }
];
const KNOWN_QUOTES = {
...
function parseTokenAmountToBN(value, decimals, label, allowZero = false) {
    ...
    return new bn_js_1.default(scaled.toFixed(0));
}
function parseSlippagePercentToBn(value) {
    const decimal = parseNonNegativeDecimal(value, "slippage percent");
    const scaled = decimal.mul(new decimal_js_1.default(SLIPPAGE_DENOMINATOR).div(100));
    if (!scaled.isInteger()) {
        throw new Error("Slippage percent supports up to 2 decimal places");
    }
    return new bn_js_1.default(scaled.toFixed(0));
}
function parsePriorityFeeMicroLamports(value) {
...
function formatRateFromBpsString(value, decimals = 2) {
    return new decimal_js_1.default(value).div(10000).toFixed(decimals);
}
function getQuoteSymbol(mint) {
...
function getQuoteCandidatesForMint(mintB) {
    if (!mintB) {
        return getQuoteCandidates();
    }
    let quoteMint;
    try {
        quoteMint = new web3_js_1.PublicKey(mintB);
    }
    catch {
...
function registerLaunchpadCommands(program) {
    ...
    launchpad
        ...
        const cluster = await (0, raydium_client_1.getConfiguredCluster)();
        const launchMintHost = (0, api_urls_1.getApiUrlsForCluster)(cluster).LAUNCH_MINT_HOST;
        const limit = Number(options.limit);
        const page = Number(options.page);
        let platforms = [];
        try {
            platforms = await (0, output_1.withSpinner)("Fetching platforms", async () => {
                const response = await fetch(`${launchMintHost}/main/platforms?page=${page}&pageSize=${limit}`);
                if (!response.ok) {
    ...
    launchpad
        ...
        if (!options.mint && !options.pool) {
            const mode = await (0, prompt_1.promptSelect)("How should the pool be located?", [
                { name: "Token mint (derive pool)", value: "mint" },
                { name: "Pool address", value: "pool" }
            ]);
            if (mode === "mint")
                options.mint = await (0, prompt_1.promptIfMissing)(undefined, "Token mint address");
            else
                options.pool = await (0, prompt_1.promptIfMissing)(undefined, "Pool address");
        }
        if (options.mint && options.pool) {
        ...
        if (options.pool && options.usd1) {
            (0, output_1.logError)("--usd1 can only be used together with --mint because --pool already selects the quote token");
            process.exitCode = 1;
            return;
        }
        let poolId;
    ...
    (0, help_1.addRichHelp)(launchpad
        .command("buy")
        ...
        .action(async (options) => {
        options.mint = await (0, prompt_1.promptIfMissing)(options.mint, "Token mint address");
        options.amount = await (0, prompt_1.promptNumberIfMissing)(options.amount, "Amount of quote token to spend", (input) => Number.isFinite(Number(input)) && Number(input) > 0 ? true : "Enter a positive amount");
        const config = await (0, config_manager_1.loadConfig)({ createIfMissing: true });
        // Validate slippage
        let slippagePercent;
        let slippageBps;
        try {
            slippagePercent = parseNonNegativeDecimal(options.slippage ?? String(config["default-slippage"]), "slippage percent");
        ...
        try {
            priorityFeeMicroLamports = parsePriorityFeeMicroLamports(options.priorityFee ?? String(config["priority-fee"]));
        }
        catch (error) {
            (0, output_1.logError)(error.message);
            process.exitCode = 1;
        ...
        if ((0, output_1.isJsonOutput)()) {
            ...
            (0, output_1.logInfo)(`Slippage: ${slippagePercent.toString()}%`);
        }
        // Confirm
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Buy failed", error, { debug: options.debug, fallback: "Buy failed" });
            process.exitCode = 1;
            return;
        }
        if ((0, output_1.isJsonOutput)()) {
    ...
    launchpad
        ...
        .action(async (options) => {
        options.mint = await (0, prompt_1.promptIfMissing)(options.mint, "Token mint address");
        options.amount = await (0, prompt_1.promptNumberIfMissing)(options.amount, "Amount of tokens to sell", (input) => Number.isFinite(Number(input)) && Number(input) > 0 ? true : "Enter a positive amount");
        const config = await (0, config_manager_1.loadConfig)({ createIfMissing: true });
        // Validate slippage
        let slippagePercent;
        let slippageBps;
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Sell failed", error, { debug: options.debug, fallback: "Sell failed" });
            process.exitCode = 1;
            return;
        }
        if ((0, output_1.isJsonOutput)()) {
    ...
    launchpad
        ...
        .addHelpText("after", "\nAuth:\n  Use --password-stdin or the interactive password prompt. --password requires --unsafe-secret-flags.\n")
        .action(async (options) => {
        options.name = await (0, prompt_1.promptIfMissing)(options.name, "Platform name");
        const config = await (0, config_manager_1.loadConfig)({ createIfMissing: true });
        // Validate fee rates
        ...
        if (!Number.isFinite(platformScale) ||
            ...
            (0, output_1.logError)("Platform, creator, and burn LP scales must be numeric percentages");
            process.exitCode = 1;
            return;
        }
        if (platformScale < 0 || platformScale > 100 || creatorScale < 0 || creatorScale > 100 || burnScale < 0 || burnScale > 100) {
            (0, output_1.logError)("Platform, creator, and burn LP scales must each be between 0 and 100");
            process.exitCode = 1;
            return;
        }
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Create platform failed", error, {
                debug: options.debug,
    ...
    (0, help_1.addRichHelp)(launchpad
        ...
        .action(async (options) => {
        options.platformId = await (0, prompt_1.promptIfMissing)(options.platformId, "Platform config address");
        options.name = await (0, prompt_1.promptIfMissing)(options.name, "Token name");
        options.symbol = await (0, prompt_1.promptIfMissing)(options.symbol, "Token symbol");
        const config = await (0, config_manager_1.loadConfig)({ createIfMissing: true });
        // Validate that either --image or --uri is provided
        if (!options.image && !options.uri) {
            const source = await (0, prompt_1.promptSelect)("How should token metadata be provided?", [
                { name: "Upload an image and generate metadata", value: "image" },
                { name: "Use an existing metadata URI", value: "uri" }
            ]);
            if (source === "image") {
                options.image = await (0, prompt_1.promptIfMissing)(undefined, "Token image path");
            }
            else {
                options.uri = await (0, prompt_1.promptIfMissing)(undefined, "Metadata URI");
            }
        }
        if (options.image && options.uri) {
            (0, output_1.logError)("Choose only one metadata source: --image or --uri");
            (0, output_1.logInfo)("Use --image to upload metadata through Pinata, or --uri to reuse existing hosted metadata.");
            process.exitCode = 1;
            return;
        }
        ...
        if (options.configId) {
            try {
                new web3_js_1.PublicKey(options.configId);
            }
            catch {
                (0, output_1.logError)("Invalid launchpad config ID address");
                process.exitCode = 1;
                return;
            }
        }
        ...
        try {
            platformInfo = await (0, output_1.withSpinner)("Fetching platform info", async () => {
                const cluster = await (0, raydium_client_1.getConfiguredCluster)();
                const launchMintHost = (0, api_urls_1.getApiUrlsForCluster)(cluster).LAUNCH_MINT_HOST;
                const response = await fetch(`${launchMintHost}/main/platforms?pageSize=100`);
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status}`);
                }
                const json = (await response.json());
        ...
        if (!platformInfo) {
            if (!(0, output_1.isJsonOutput)()) {
                (0, output_1.logInfo)(`Platform not found in API, using on-chain ID: ${platformId.toBase58()}`);
            }
        }
        ...
        if (options.buyAmount &&
            ...
            (0, output_1.logError)(`--buy-amount is currently interpreted in SOL, but config ${selectedConfig.key.pubKey} uses quote token ${selectedConfig.mintInfoB.symbol}`);
            (0, output_1.logInfo)("Omit --buy-amount or choose a SOL-quoted launchpad config.");
            process.exitCode = 1;
            return;
        }
        ...
        if ((0, output_1.isJsonOutput)()) {
            ...
            (0, output_1.logInfo)(`  Platform: ${platformInfo?.name ?? "Custom"} (${platformId.toBase58()})`);
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Failed to build transaction", error, {
                debug: options.debug,
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Create launchpad failed", error, {
                debug: options.debug,
                fallback: "Unknown error - run with --debug for details"
            });
            process.exitCode = 1;
    ...
    launchpad
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Claim fees failed", error, { debug: options.debug, fallback: "Claim fees failed" });
    ...
    launchpad
        ...
        const knownQuoteTokens = KNOWN_QUOTE_TOKENS.map((token) => ({
            symbol: token.symbol,
        ...
        for (const token of tokensToCheck) {
            ...
            const vaultPda = (0, raydium_sdk_v2_1.getPdaPlatformVault)(raydium_sdk_v2_1.LAUNCHPAD_PROGRAM, platformId, mintB).publicKey;
            try {
                ...
                if (!(0, output_1.isJsonOutput)()) {
                    (0, output_1.logInfo)(`  ${token.symbol}: ${displayAmount} (${amount.toString()} raw)`);
    ...
    launchpad
        ...
        for (const token of tokensToCheck) {
            ...
            const vaultPda = (0, raydium_sdk_v2_1.getPdaCreatorVault)(raydium_sdk_v2_1.LAUNCHPAD_PROGRAM, owner.publicKey, mintB).publicKey;
            try {
                // Fetch token account info
                const accountInfo = await raydium.connection.getAccountInfo(vaultPda);
                if (!accountInfo) {
    ...
    launchpad
        ...
        try {
            ...
            (0, output_1.logErrorWithDebug)("Claim creator fees failed", error, { debug: options.debug, fallback: "Claim creator fees failed" });
            // Check if it's a "no fees to claim" scenario
